...
@SpringBootTest
@EmbeddedKafka(controlledShutdown = true, topics = { KafkaConfig.DRINK_REQUEST_ICE_COLD_TOPIC, KafkaConfig.DRINK_PREPARED_TOPIC }, partitions = 1, brokerProperties = { "log.dir=target/embedded-kafka" } )
public class IceColdDrinkRequestListenerTest {
	@Autowired KafkaListenerEndpointRegistry kafkaListenerEndpointRegistry;
	@Autowired IceColdDrinkRequestListener iceColdDrinkRequestListener;
	...
	void listen() {
		DrinkRequestEvent drinkRequestEvent = DrinkRequestEvent.builder()
			.beerOrderLine(BeerOrderLineDTO.builder()
				...
				.build())
			.build();
		iceColdDrinkRequestListener.listen(drinkRequestEvent);
